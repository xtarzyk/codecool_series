...
@app.route("/third_season")
def third_season():
    seasons = queries.get_third(3)
    return render_template('third_season.html', seasons=seasons)


@app.route('/filter')
def filter_rating():
    # rating = dict(request.form)
    # session["rating"] = rating["filter"]
    rating = float(request.args.get('filter'))
    search_result = queries.rating(3, rating)
    return render_template('third_season.html', search_result=search_result)
